...
const DEGREE_OF_SIMILARITY = 0.2;
...
const recommend = function() {  
  pool.getConnection(function(err, connection) {
    ...
    connection.query(
      `
        SELECT s.contactId, s.itemId, s.title, s.url, s.imgURL, s.seller, s.category
...
function addSimilarSuggestions(similarUsers, savedSuggestions, existingSimilar, formattedTags) {
    ...
    similarUsers[key].forEach((contactId) => {
      savedSuggestions[key].forEach((suggestion) => {
        if(Object.keys(existingSimilar).length === 0 || !existingSimilar[contactId] || !existingSimilar[contactId].includes(suggestion.itemId)) {
          queryObject.push(
            [contactId, suggestion.itemId, suggestion.title, suggestion.category, suggestion.url, suggestion.imgURL, suggestion.seller, formattedTags[contactId][0].userCognitoId, 0, Date.now(), 1]
          )
        }
      })
    })
  ...
  if(queryObject.length > 0) {
    pool.getConnection(function(err, connection) {
      ...
      connection.query(
        `
          INSERT INTO gifter.suggestions (contactId, itemId, title, category, url, imgURL, seller, userCognitoId, saved, createdAt, similarSuggestion) VALUES ?
        `, [queryObject],
        function(error, results, fields) {
          if(error) console.log(error);
          connection.release();
